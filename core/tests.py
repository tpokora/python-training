...
from core.models import User, UserConfiguration, Configuration
...
class CoreTests(TestCase):
    ...
    @staticmethod
    def create_configuration_for_user(user):
        user.create_user_configuration()
        configuration = user.get_user_configuration()
    ...
    def test_configuration__str__(self):
        ...
        expected_conf_str = "user: '{}', configuration: {}".format(configuration.user.__str__(),
                                                                   configuration.configuration)

        self.assertEqual(configuration.__str__(), expected_conf_str, True)


class UserConfigurationTests(TestCase):

    def test_create_configuration_for_user(self):
        user = CoreTests.create_test_user()
        user_configuration = CoreTests.create_configuration_for_user(user)
        user_configuration = user.get_user_configuration()

        self.assertEqual(user_configuration.user.pk, user.pk, True)
        self.assertEqual(user_configuration.configuration, Configuration.DEFAULT_CONFIGURATION, True)
